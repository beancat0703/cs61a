...
def boar_brawl(player_score, opponent_score):
    ...
    """
    # BEGIN PROBLEM 2
    opponent_score_ten_digit = opponent_score // 10
    opponent_score_ten_digit %= 10
    player_score_single_digit = player_score % 10
    score = abs(opponent_score_ten_digit - player_score_single_digit)
    if score == 0:
        return 1
    else:
        return 3 * score
...
def make_averaged(original_function, times_called=1000):
    ...
    def average(*args):
            ...
            i = times_called
            while i > 0:
                score = score + original_function(*args)
                i -= 1
...
def max_scoring_num_rolls(dice=six_sided, times_called=1000):
    ...
    score_com = 0
    while i <= 10:
        score_com = make_averaged(roll_dice, times_called)(i, dice)
        if  score_com > score:
            k = i
        score = score_com
        i += 1
...
def sus_strategy(score, opponent_score, threshold=11, num_rolls=6):
    ...
    Suss Fuss rules."""
    # BEGIN PROBLEM 11
    score_change = sus_update(0, score, opponent_score, dice=six_sided)
    sub = score_change - score
    if sub >= threshold:
        return 0
    else:
        return num_rolls
